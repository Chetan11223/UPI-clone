...
type PaymentStep = 'amount' | 'recipient' | 'review' | 'processing' | 'result';

export function PayPage() {
  const navigate = useNavigate();
  const { user, accounts, setPaymentFlow, paymentFlow, addTransaction } = useStore();
  
  const [step, setStep] = useState<PaymentStep>('amount');
  const [amount, setAmount] = useState('');
  const [recipientType, setRecipientType] = useState<'vpa' | 'phone' | 'qr'>('vpa');
  ...
  const renderAmountStep = () => (
    <div className="space-y-6">
      ...
      <AmountInput
        ...
        label="Amount to Pay"
        placeholder="0.00"
        maxAmount={defaultAccount?.balance || 100000}
      />
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { type: 'qr' as const, icon: <QrCode className="w-5 h-5" />, label: 'QR Code' },
          ].map((option) => (
            <button
              key={option.type}
        ...
        {recipientType === 'qr' && (
          <div className="text-center py-8">
            <div className="w-16 h-16 bg-silver-100 dark:bg-silver-800 rounded-full flex items-center justify-center mx-auto mb-4">
              <QrCode className="w-8 h-8 text-silver-400" />
            </div>
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardContent className="p-6 space-y-4">
          ...
          <div className="flex items-center justify-between">
            <span className="text-silver-600 dark:text-silver-400">From Account</span>
            <div className="text-right">
              <p className="font-medium text-silver-900 dark:text-silver-100">
        ...
        <>
          ...
          <Card>
            <CardContent className="p-6 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-silver-600 dark:text-silver-400">Amount Paid</span>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-mono text-silver-500 dark:text-silver-500">
                  {paymentResult.transaction?.referenceId}
                </span>
              </div>
            </CardContent>
          </Card>
